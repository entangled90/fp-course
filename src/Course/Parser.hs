...
(|||) p1 p2 = P ( \i -> 
    let res = (parse p1 i) 
    in if (isErrorResult res) then parse p2 i else res
  )
...
thisMany ::
  ...
  -> Parser a
  -> Parser (List a)
thisMany n pa = if (n == 0) then valueParser Nil else (\a -> (((:.) a)) <$> (thisMany (n-1) pa)) =<< pa
...
firstNameParser ::
  Parser Chars
firstNameParser =  (singleton <$> upper) <> (list lower)
...
surnameParser ::
  Parser Chars
surnameParser = (singleton <$> upper) <> (thisMany 5 lower) <> (list lower)
...
smokerParser ::
  Parser Bool
smokerParser = const True <$> (is 'y' ||| is 'n')
...
phoneParser ::
  Parser Chars
phoneParser = (singleton <$> digit) <> phoneBodyParser <> (pure <$> (is '#'))
